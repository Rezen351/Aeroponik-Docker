  ...
  previousYear(fromYear) {
    ...
    return (_a = this.items.reduce((years, item) => {
      ...
      if (item.any) years.push(fromYear);else if (item.single) {
        ...
        if (end < fromYear) years.push(end);else {
          const prevYear = end - Math.ceil((end - fromYear) / item.step) * item.step;
          if (prevYear >= ((_d = (_c = item.range) === null || _c === void 0 ? void 0 : _c.from) !== null && _d !== void 0 ? _d : minValidYear)) years.push(prevYear);
        }
      }
      return years;
...
class CronosExpression {
  ...
  previousDate(beforeDate = new Date()) {
    ...
    if (this.skipRepeatedHour && prevHourRepeated) {
      return this._previous(fromCronosDate.copyWith({
        minute: 0,
        second: -1
      }), false).date;
    }
    if (this.missingHour === 'offset' && thisHourMissing) {
      const previousDate = this._previous(fromCronosDate.copyWith({
        hour: fromCronosDate.hour + 1
      })).date;
    ...
    let {
      date: previousDate,
      cronosDate: previousCronosDate
    } = this._previous(fromCronosDate);
    if (this.missingHour !== 'offset' && previousCronosDate && previousDate) {
      const previousDatePrevHourTimestamp = previousCronosDate.copyWith({
        hour: previousCronosDate.hour - 1
      }).toDate(this.timezone).getTime();
      if (previousDatePrevHourTimestamp === previousDate.getTime()) {
        if (this.missingHour === 'insert') {
          return previousCronosDate.copyWith({
            minute: 59,
            second: 59
          }).toDate(this.timezone);
        }
        // this.missingHour === 'skip'
        return this._previous(previousCronosDate.copyWith({
          minute: 0,
          second: 0
        })).date;
      }
    ...
    if (!this.skipRepeatedHour) {
      if (prevHourRepeated && (!previousDate || previousDate.getTime() < fromTimestamp - hourinms)) {
        previousDate = this._previous(fromCronosDate.copyWith({
          minute: 59,
          second: 59
        }), false).date;
      }
  ...
  _previous(date, before = true) {
    const previousDate = this._previousYear(before ? date.copyWith({
      second: date.second - 1
    }) : date);
    return {
  ...
  _previousDay(fromDate) {
    ...
    while (!previousDate) {
      ...
      previousDate = this._previousHour(previousDay === fromDate.day ? fromDate : new CronosDate(fromDate.year, fromDate.month, previousDay, 23, 59, 59));
      previousDayIndex--;
    }
    return previousDate;
  }
  _previousHour(fromDate) {
    ...
    while (!previousDate) {
      ...
      previousDate = this._previousMinute(previousHour === fromDate.hour ? fromDate : new CronosDate(fromDate.year, fromDate.month, fromDate.day, previousHour, 59, 59));
      previousHourIndex--;
    }
    return previousDate;
  }
  _previousMinute(fromDate) {
    ...
    while (!previousDate) {
      ...
      previousDate = this._previousSecond(previousMinute === fromDate.minute ? fromDate : new CronosDate(fromDate.year, fromDate.month, fromDate.day, fromDate.hour, previousMinute, 59));
      previousMinuteIndex--;
    }
    return previousDate;
  }
